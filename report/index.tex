...
\end{lstlisting}
    ...
    Across 64 fully loaded cores of an AMD EPYC 7713 with 64~GB of RAM, the complete fuzzer in its differential fuzzing modes achieves approximately 6000 executions per second. To reduce the impact of bottlenecks from starting many processes, the following experiments are run on single-core instances.

    \subsubsection{Performance Implications of Individual Parts}
    \label{PerformanceIndividualParts}

    The fuzzer, as described in Section~\ref{Implementation}, contains multiple parts that may contribute to the overall runtime of a single fuzzer execution. LibAFL contains performance collection code, which the results in Listing~\ref{listing:LibAFLIntrospection} and shows that the vast majority of runtime is contributed by the target execution. The remaining measured expensive functions are the mutations, since many are executed on each run, and coverage handling, which in total contribute less than 0.5~\% to the runtime of the fuzzer.
...
\end{lstlisting}
    ...
    The fuzzer in its full differential mode on a single thread reports approximately 250 executions per second, or about a runtime of 0.04~s per execution. The \code{TargetExecution} described in Listing~\ref{listing:LibAFLIntrospection} contains multiple steps: the input is written to a file to feed to the binary under test, which is then run, its output is captured, and coverage information is copied back to the fuzzer.

    Figure~\ref{fig:coverage-instrumentation} compares the performance of GNU's and uutils' version of coreutils with different approaches to coverage instrumentations. First, it shows that the performance of the two implementations without instrumentation is comparable, with the uutils version being slightly slower.

    The compared modes include tainting, or marking which edge has been executed, and counting, which counts how often an edge is passed. The latter requires an additional read access on each edge, and is thus slightly less performant than the first.

    Adding instrumentation to GNU's version of coreutils adds approximately a third to its runtime, while this factor is much greater for uutils' version. Additional experiments have shown, that compiling it with its \code{release-small}, or size-optimized profile is most performant. This shows that binary size, and with it edge count, is the primary factor in a Rust binary. Why clang produces many more edges in Rust code compared to C code could not be evaluated in this project, but may be interesting follow-up work.
    ...
    Since pipes have a maximum size in Linux, the input cannot always be passed to the program under test directly. To work around this limitation, the fuzzer will write the input to a temporary file in a in-memory file system and pass this file to the program under test. Figure~\ref{fig:stdin-types} shows the performance impact of doing this. While the performance is small for small input data, this is not necessarily the case for larger input data, as explored in Figure~\ref{fig:file-write}.
    ...
    Figure~\ref{fig:dynamic-file-stdin} shows how inputs of different sizes impact the performance of \code{base64} in total. This includes the additional runtime shown in Figure~\ref{fig:file-write} and shows that compared to the runtime of \code{base64}, the impact of writing the data to a file is an order of magnitude smaller and thus acceptable.
    ...
    Performance of shared memory as provided by the \code{MmapShmem} module in LibAFL is shown in Figure~\ref{fig:shmem} to be comparable to writing an input file and thus does not incur problematic overhead. Finally, Figure~\ref{fig:preloads} presents the performance implications of hooking into the functions necessary for this fuzzer. Compared to the baseline, the first option simply hooks into \code{libc_start_main} and calls the program's main function unaltered. The second option also hooks into \code{libc_start_main}, but includes a hook for \code{rtld_fini} in its call to the main function, which in this example only calls the original \code{rtld_fini}, but is used in the fuzzer to copy coverage information back to the fuzzer.
    ...
    Overall, the fuzzers performance can be explained almost entirely by a combination of these experiments, with the remaining runtime being introduced by minor additional logic such as output capturing.
    ...
    In an evaluation run lasting 24 hours and including almost 100 million executions, the fuzzer was unable to find any errors in \code{base64} from either version of coreutils, neither in its individual or differential mode. However, \code{base64} fits into 212 lines of code according to \code{gcov}, and considering the wide-spread usage of \code{base64} and the number of people who have looked at the code, it is very unlikely that there are any errors in the code base to begin with.
    ...
    This is not the first project performing differential fuzzing nor the first project fuzzing coreutils. This section introduces previous works and discusses their limitations, as compared to this project.

    \subsection{uutils' Fuzzing Sub-Project}

    The only other project performing differential fuzzing on coreutils the author could find is within the uutils coreutils repository. It contains code to fuzz 16 programs, 11 of which perform differential fuzzing against the system installation of the appropriate program from GNU coreutils.

    libFuzzer is used as a fuzzing backend, which integrates with cargo and provides coverage instrumentation. However, the interface of libFuzzer is currently limited to the \code{fuzz_target} macro, which only supports input types in the shape of simple byte arrays. In the project in its current state, this input is not used in combination with differential fuzzing. For those programs, the input is manually randomly generated, thus loosing any advantage that coverage-guided fuzzing introduces.

    Only for the non-differential fuzzers, the input provided by libFuzzer is used. 4 of 5 fuzzing binaries in this category do not test entire programs but instead imported parsing routines, with \code{date} being the only program from the coreutils suite where actual coverage-guided fuzzing on a full program from coreutils is performed. The command line arguments are generated from the byte array input by splitting at \code{NULL} bytes. This is not a generally viable strategy, as is argued in Section~\ref{Environment}.

    Overall, compared to uutils' fuzzing sub-project, this work performs both coverage-guided and differential fuzzing at the same time, while utilizing the flexibility gained by building on top of LibAFL to increase performance and ensure environment protection and thus reliable testing.
    ...
    \begin{enumerate}
        ...
        \item Finally, the fuzzer in its differential mode is able to find artificially introduced nontrivial logic errors in the different implementations, undiscoverable by its non-differential mode. It explores all program parts except for those excluded based on environmental protection reasons as described above.
    \end{enumerate}
    ...
    While the fuzzer as introduced in the previous sections was already able to produce results in the form of prove-of-concept work, it is subject to a set of important limitations.
    ...
    This project introduces a new greybox differential fuzzer for coreutils. It builds on top and includes many advanced algorithms implemented in LibAFL to generate inputs while ensuring a stable testing environment.

    A new approach to transferring coverage information from a program executed as a separate process is used, where the programs under test are extended with an interface to dynamically access coverage information without any changes to the source code. By dynamically loading and linking helper binaries during execution, this system provides coverage information idiomatically to be read by LibAFL's coverage guidance modules.

    Due to incomplete functionality in LibAFL, the fuzzer produced in this project could only be evaluated on \code{base64}, where it achieved full coverage, but did not discover any software errors. More extensive tests will be required as a base for a full evaluation once the missing logic has been added to LibAFL.

    In the interest of open science, this project is released open-source. During development, hundreds of lines of code have been introduced to the upstream project, with further major changes already in discussion. The source code of the project is publicly available at \href{https://github.com/riesentoaster/coreutils-differential-fuzzing}{https://github.com/riesentoaster/coreutils-differential-fuzzing}.
