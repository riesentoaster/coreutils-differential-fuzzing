...
\end{lstlisting}
    ...
    Only for the non-differential fuzzers, the input provided by libFuzzer is used. 4 of 5 fuzzing binaries in this category do not test entire programs but instead imported parsing routines, with \code{date} being the only program from the coreutils suite where actual coverage-guided fuzzing is performed. The command line arguments are generated from the byte array input by splitting at \code{NULL} bytes. This is not a generally viable strategy, as is argued in Section~\ref{Environment}.
