...
\end{lstlisting}
    ...
    \begin{enumerate}
        ...
        \item Finally, the fuzzer in its differential mode finds nontrivial logic errors in the different implementations, undiscoverable by its non-differential mode. It explores all program parts except for those excluded based on environmental protection reasons as described above.
    \end{enumerate}
    ...
    While evaluation is currently only possible in a very limited fashion due to incomplete functionality in LibAFL, tests on \code{base64} already found a bug in GNU's coreutils, promising decent yields on more extended tests.
